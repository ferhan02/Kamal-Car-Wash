...
$overtime_pay = (float) ($overtime['overtime_pay'] ?? 0);
$overtime_hours = (float) ($overtime['overtime_hours'] ?? 0);
...
$absent = (int) ($attendance['absent_days'] ?? 0);
$totalDays = $present + $absent;
$attendancePercent = $totalDays > 0
    ? (int) round(($present / $totalDays) * 100)
    : 0;
...
.pay-hero {
    ...
    background: linear-gradient(
        135deg,
        color-mix(in srgb, var(--staff-blue) 90%, #002f62),
        color-mix(in srgb, var(--staff-purple) 60%, var(--staff-blue))
    );
    color: #fff;
}
...
.ring {
    ...
    background: conic-gradient(
        var(--staff-green) 0 calc(var(--attendance-percent) * 1%),
        var(--staff-surface-2) calc(var(--attendance-percent) * 1%) 100%
    );
}
...
if (kcw_admin_role($staff['staff_job'])) {
    include __DIR__ . '/../includes/admin_toolbar.php';
}
?>
...
<main class="staff-main">
    ...
    <section class="ios-card pay-hero">
        <div>
            ...
            <strong>RM <?= number_format($net, 2) ?></strong>
    ...
    <section class="pay-grid">
        <article class="ios-card">
            ...
            <div class="ios-card-body breakdown-list">
                <div class="breakdown-row">
                    ...
                    <strong>RM <?= number_format($basic, 2) ?></strong>
                </div>

                <div class="breakdown-row">
                    <span>Allowance</span>
                    <strong>+ RM <?= number_format($allowance, 2) ?></strong>
                </div>

                <div class="breakdown-row">
                    <span>Approved overtime (<?= number_format($overtime_hours, 2) ?> h)</span>
                    <strong>+ RM <?= number_format($overtime_pay, 2) ?></strong>
                </div>

                <div class="breakdown-row">
                    <span>Bonus</span>
                    <strong>+ RM <?= number_format($bonus, 2) ?></strong>
                </div>

                <div class="breakdown-row">
                    <span>Deduction</span>
                    <strong style="color: var(--staff-red);">
                        − RM <?= number_format($deduction, 2) ?>
                    </strong>
                </div>

                <div class="breakdown-row">
                    <span>Gross pay</span>
                    <strong>RM <?= number_format($gross, 2) ?></strong>
                </div>
            </div>
        </article>
        ...
        <article class="ios-card">
            ...
            <div class="ios-card-body attendance-ring">
                <div
                    class="ring"
                    style="--attendance-percent: <?= $attendancePercent ?>;"
                    aria-label="Attendance rate <?= $attendancePercent ?> percent"
                >
